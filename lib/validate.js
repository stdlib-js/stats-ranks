...
function validate( opts, options ) {
	if ( !isObject( options ) ) {
		return new TypeError( format( '1Lp2V,FD', options ) );
	}
	if ( hasOwnProp( options, 'encoding' ) ) {
		opts.encoding = options.encoding;
		if ( !isArray( opts.encoding ) ) {
			return new TypeError( format( '1Lp8Z,GX', 'encoding', opts.encoding ) );
		}
	}
	if ( hasOwnProp( options, 'method' ) ) {
		opts.method = options.method;
		if ( !isString( opts.method ) || !contains( METHODS, opts.method ) ) {
			return new TypeError( format( '1Lp2X,3g,4S,GD,Gg,Jm', 'method', METHODS.join( '", "' ), opts.method ) );
		}
	}
	if ( hasOwnProp( options, 'missing' ) ) {
		opts.missing = options.missing;
		if ( !isString( opts.missing ) || !contains( MISSING, opts.missing ) ) {
			return new TypeError( format( '1Lp2X,3g,4S,GD,Gg,Jm', 'missing', MISSING.join( '", "' ), opts.missing ) );
		}
	}
	return null;
}
